		...
		ap_uint<1>						  &lut_miss)
{
	...
	static Queue<ap_axis> write_queue;

	static Queue<dest_port_type> dest_queue;
	...
	s_axis_tready = !write_queue.isNearlyFull();

	ap_uint<1> is_nearly_full = !write_queue.isNearlyFull();
	...
	wren = (s_axis_tvalid & !write_queue.isNearlyFull());
		...
		ap_uint<1>  				   &m_axis_tlast)
{
	...
	static Queue<ap_axis> read_queue;

	static Queue<dest_port_type> scr_queue;
	...
	ap_axis transfer = read_queue.Front();

	ap_uint<1> send_packet;

	ap_uint<1> dst_port_rd;

	ap_uint<1> infifo_empty = read_queue.isEmpty();
	...
	Send_packet(sth,scr_queue.Front(),scr_queue.isEmpty(),~infifo_empty,m_axis_tready,transfer.packet.range(LASTINBASE,LASTINBASE),reset,send_packet,dst_port_rd);
	...
	if(infifo_ren)read_queue.Dequeue();

	if(dst_port_rd)scr_queue.Dequeue();
